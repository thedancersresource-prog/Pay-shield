...
const STORAGE_KEY = "payshield_v2";
...
cashapp: "$NovaStage",
venmo: "@nova-tips",
instagram: "@nova.official",
},
venues: [
{ club: "Gold Room", city: "Atlanta, GA", days: ["Thu", "Fri", "Sat"], hours: "9pm-2am", notes: "VIP available on request" },
{ club: "Onyx", city: "Atlanta, GA", days: ["Tue", "Wed"], hours: "8pm-1am", notes: "" },
],
},
},
};
...
All tips are voluntary personal transactions between you and the performer.
</div>
<div className="disclaimer-clause">
You confirm you are of legal age to make this transaction in your jurisdiction.
</div>
<div className="disclaimer-clause">
All transactions are final and non-refundable. Tipping is a gift, not a purchase.
</div>
<div className="disclaimer-clause">
Initiating a chargeback or payment dispute for a voluntary tip may constitute fraud and can result in legal action.
</div>
<div className="disclaimer-clause">
No personal information is collected or stored from this interaction.
</div>
<div className="disclaimer-clause">
This platform (TAPPED / SI Parker Technologies) is not a party to this transaction and assumes no liability.
</div>
</div>
<div className="timestamp">Acknowledged: {ts}</div>
<button className="btn btn-primary" onClick={onAccept}>
I Understand — Continue to Tip
</button>
</div>
</div>
);
...
const [accepted, setAccepted] = useState(!profile.requireDisclaimer);
const [selectedPlatform, setSelectedPlatform] = useState(null);

const activePay = PAY_PLATFORMS.filter(p => profile.pay[p.id]);
...
<DisclaimerModal
name={profile.name}
onAccept={() => { setAccepted(true); setShowDisclaimer(false); }}
/>
)}
...
<div className="public-name">{profile.name}</div>
...
Protected by TAPPED · SI Parker Technologies
</div>
</div>
...
function updatePay(id, val) {
setDraft(prev => ({ ...prev, pay: { ...prev.pay, [id]: val } }));
setSaved(false);
}
...
reader.onload = ev => updateDraft("photo", ev.target.result);
reader.readAsDataURL(file);
}

function addVenue() {
updateDraft("venues", [...(draft.venues || []), { ...BLANK_VENUE }]);
}

function updateVenue(i, key, val) {
const updated = [...(draft.venues || [])];
updated[i] = { ...updated[i], [key]: val };
updateDraft("venues", updated);
}

function toggleVenueDay(i, day) {
const venue = draft.venues[i];
const days = venue.days.includes(day)
? venue.days.filter(d => d !== day)
: [...venue.days, day];
updateVenue(i, "days", days);
}
...
{draft.photo ? <img src={draft.photo} alt={draft.name} /> : draft.name?.[0] || "?"}
</div>
<input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handlePhoto} />
<div className="profile-info">
<h2>{draft.name || username}</h2>
...
<div className="section-sub">This is what fans see when they tap your chip</div>
</div>
<div className="field">
<label>Stage Name</label>
<input value={draft.name || ""} onChange={e => updateDraft("name", e.target.value)} placeholder="Your stage name" />
</div>
<div className="field">
<label>Bio</label>
...
<>
<div>
<div className="section-title">Pay Links</div>
<div className="section-sub">Add your tip platforms — leave blank to hide</div>
</div>
<div className="card">
{PAY_PLATFORMS.map(p => (
...
<input
value={draft.pay?.[p.id] || ""}
onChange={e => updatePay(p.id, e.target.value)}
...
<div className="section-sub">Where fans can find you</div>
</div>
...
<div>
<div className="venue-name">{v.club || "New Venue"}</div>
...
<input value={v.club} onChange={e => updateVenue(i, "club", e.target.value)} placeholder="Club name" />
</div>
<div className="field" style={{ marginBottom: 8 }}>
<label>City</label>
<input value={v.city} onChange={e => updateVenue(i, "city", e.target.value)} placeholder="City, State" />
</div>
<div>
...
{ALL_DAYS.map(d => (
<button key={d} className={`day-chip${v.days.includes(d) ? " selected" : ""}`} onClick={() => toggleVenueDay(i, d)}>{d}</button>
))}
</div>
</div>
<div className="field" style={{ marginBottom: 8 }}>
<label>Hours</label>
<input value={v.hours} onChange={e => updateVenue(i, "hours", e.target.value)} placeholder="e.g. 9pm–2am" />
</div>
<div className="field" style={{ marginBottom: 0 }}>
<label>Notes</label>
<input value={v.notes} onChange={e => updateVenue(i, "notes", e.target.value)} placeholder="VIP info, booking notes..." />
</div>
</div>
))}
</div>
...
<div className="section-sub">Control what fans see</div>
</div>
<div className="card">
<div className="toggle-row">
<div className="toggle-label">
<span>Tip Disclaimer</span>
<small>Require acknowledgment before pay links open</small>
</div>
<label className="toggle">
<input type="checkbox" checked={!!draft.requireDisclaimer} onChange={e => updateDraft("requireDisclaimer", e.target.checked)} />
...
<small>Hide your name from analytics</small>
</div>
<label className="toggle">
<input type="checkbox" checked={!!draft.anonymousMode} onChange={e => updateDraft("anonymousMode", e.target.checked)} />
<span className="toggle-slider" />
</label>
</div>
</div>
...
onLogin(u.toLowerCase(), user.profile);
} else {
...
<input value={u} onChange={e => { setU(e.target.value); setErr(""); }} placeholder="your username" onKeyDown={e => e.key === "Enter" && handleLogin()} />
</div>
<div className="field">
<label>Password</label>
<input type="password" value={p} onChange={e => { setP(e.target.value); setErr(""); }} placeholder="••••••••" onKeyDown={e => e.key === "Enter" && handleLogin()} />
</div>
...
<>
<style>{styles}</style>
<PublicProfile profile={user.profile} />
</>
);
}
}
...
const users = loadUsers();
users[session.username] = { ...users[session.username], profile: updatedProfile };
saveUsers(users);
...
<>
<style>{styles}</style>
<PublicProfile profile={session.profile} onBack={() => setPreview(false)} />
</>
);
}
